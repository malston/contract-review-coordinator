"""The linchpin: the send_email PreToolUse gate reads REAL completion state.
...
def test_blocks_when_no_review_exists():
    decision = pre_tool_send_email(_email(), _state(None))
    assert decision.allowed is False
    assert "not complete" in decision.reason


def test_blocks_when_review_still_pending():
    decision = pre_tool_send_email(_email(), _state(_complete_review(status="pending")))
    assert decision.allowed is False
    assert "not complete" in decision.reason
...
def test_blocks_when_review_is_for_a_different_document():
    ...
    decision = pre_tool_send_email(_email(), _state(stale))
    assert decision.allowed is False
...
def test_blocks_when_email_cites_an_unverified_clause():
    ...
    decision = pre_tool_send_email(email, _state(_complete_review()))
    assert decision.allowed is False
...
def test_model_claiming_it_reviewed_does_not_pass_the_gate():
    ...
    decision = pre_tool_send_email(email, _state(None))
    assert decision.allowed is False
...
def test_allows_when_review_complete_doc_matches_and_clauses_verified():
    decision = pre_tool_send_email(_email(), _state(_complete_review()))
